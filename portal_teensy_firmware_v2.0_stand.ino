...
const uint8_t CALIBRATED = 7; // откалибровано 0b00000111 -  Z, Y, X

// направление движения к концевикам: 1 - в положительную; 0 - в отрицательную
const uint8_t CALIBRATION_DIR = 0b00000110; // Z, Y, X
...
void setup()
{
	...
	X.setStepsPerRevolution(10000)
		.setUnitsPerRevolution(6.667F)   // можно уточнить до 6.676, если хотите
...
void calibratePhase5() // точная калибровка осей X, Y, Z
{
	...
	if ((digitalReadFast(PIN_LIMIT_SWITCH[0]) == LEVEL_LIMIT_SWITCH[0]) || 
		...
		(digitalReadFast(PIN_LIMIT_SWITCH[2]) == LEVEL_LIMIT_SWITCH[2])) // ошибка, одна или несколько осей не двигаются / один или несколько датчиков не работают
	{
		trySetStatus(StatusMain::ERROR);
		addDataToOutQueue(COMMAND_CALIBRATE_ALL + RESULT_ERROR_CALIBRATION_SENSOR);
		return;
